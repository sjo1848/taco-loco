...
| Cursor precision | D1 adapter and public projection | The public `version` is an opaque SHA-256 cursor derived from the bearer token and internal sequence; sequence values remain exact internally within the adapter's safe integer boundary, unsafe values fail explicitly rather than being silently rounded, and unsafe external cursors are not sent to D1 | Cursor/privacy tests and runtime evidence |
