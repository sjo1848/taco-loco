...
export async function applyOrderWorkflowAction(input: unknown, actorId: string) {
  ...
  if (isD1Runtime) {
    ...
    await db.$transaction(async (tx: { order: typeof db.order; orderEvent: typeof db.orderEvent }) => {
      ...
      const result = await tx.order.updateMany({ where: { id: parsed.orderId, status: current.status, verificationStatus: parsed.action === "EXPIRE_PENDING" ? "PENDING" : undefined, paymentStatus: parsed.action === "REPORT_PAYMENT" ? "PENDING" : parsed.action === "REJECT_PAYMENT" ? { in: ["PENDING", "REPORTED"] } : undefined, refundStatus: parsed.action === "REQUIRE_REFUND" ? "NOT_REQUIRED" : parsed.action === "MARK_REFUNDED" ? "REQUIRED" : undefined }, data: { ...data, updatedById: actorId } });
