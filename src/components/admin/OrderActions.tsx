...
type WorkflowAction = "EXPIRE_PENDING" | "MARK_NO_SHOW" | "REPORT_PAYMENT" | "REJECT_PAYMENT" | "REQUIRE_REFUND" | "MARK_REFUNDED";
...
export function OrderActions({ orderId, transitions, fulfillment, verificationStatus = "VERIFIED", paymentStatus = "NOT_REQUIRED", refundStatus = "NOT_REQUIRED", status = "RECEIVED" }: Props) {
  ...
  function transition(toStatus: OrderStatus) { if (toStatus === "CANCELLED" && !reason.trim()) { setError("Indicá el motivo de cancelación."); return; } void send({ toStatus, confirmPayment: fulfillment === "DELIVERY" && toStatus === "CONFIRMED", reason: toStatus === "CANCELLED" ? reason : undefined }); }
