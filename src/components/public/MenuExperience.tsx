...
import { buildWhatsAppAppUrl, buildWhatsAppMessage, buildWhatsAppMessageWithCheckout, buildWhatsAppUrl, createClientReference, addSelectionLine, lineId, removeSelectionLine, selectionCount, selectionTotal, updateSelectionQuantity, type CheckoutDetails, type SelectionLine } from "@/modules/selection/model";
...
type Menu = { settings: { businessName: string; whatsappUrl: string; deliveryEnabled: boolean; deliveryFeeAmount: number; operatingContext: { isOpen: boolean; label: string; detail: string } }; categories: Array<{ id: string; name: string; slug: string; products: Product[] }> };
type CompletedOrder = { number: number; lines: SelectionLine[]; total: number; whatsappHref: string; whatsappAppHref: string; trackingHref: string };
...
export function MenuExperience({ menu }: { menu: Menu }) {
  ...
  const [checkout, setCheckout] = useState<CheckoutDetails>({ fulfillment: "PICKUP" });
  ...
  async function submitOrderIntent(event: MouseEvent<HTMLAnchorElement>) {
    ...
    try {
      const response = await fetch("/api/orders/intents", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ clientReference, fulfillment: checkout.fulfillment, deliveryAddress: checkout.deliveryAddress || null, deliveryReference: checkout.deliveryReference || null, transferHolderName: checkout.transferHolderName || null, lines: lines.map((line) => ({ productId: line.productId, quantity: line.quantity, modifiers: line.modifiers })) }) });
      if (!response.ok) throw new Error("ORDER_INTENT_FAILED");
      const result = await response.json() as { order: { orderNumber: number; totalAmount: number }; tracking: { path: string; url: string } | null };
      if (!result.tracking) throw new Error("TRACKING_UNAVAILABLE");
      const preparedMessage = buildWhatsAppMessageWithCheckout(lines, menu.settings.businessName, result.order.orderNumber, checkout, result.tracking.url);
      ...
      const preparedOrder: CompletedOrder = { number: result.order.orderNumber, lines, total: result.order.totalAmount, whatsappHref: preparedWhatsappHref, whatsappAppHref: preparedWhatsappAppHref, trackingHref: result.tracking.path };
  ...
  return <main className="public-menu"><section className="brand-pattern brand-pattern--hero brand-hero"><BrandHeader businessName={menu.settings.businessName} /><div className="brand-hero__copy"><p className="eyebrow">Menú digital</p><h1>Elegí tu próximo antojo</h1><p>Sabores para compartir, pedir y volver a disfrutar.</p></div></section><CategoryNav categories={visibleCategories} /><p className="sr-only" aria-live="polite">{feedbackMessage || (addedProductId ? "Producto agregado al pedido" : "")}</p>
